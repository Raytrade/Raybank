...
func main() {
	...
	r.StaticFile("/bank.html", "./bank.html")
	...
	r.POST("/transfer/initiate", func(c *gin.Context) {
		...
		tx.Commit(context.Background()); c.JSON(200, gin.H{"status": "PENDING", "tx_id": tid})
	})
	r.POST("/transfer/settle/:tx_id", func(c *gin.Context) {
		...
		tx.Commit(context.Background()); c.JSON(200, gin.H{"status": "SUCCESS"})
	})
	p := os.Getenv("PORT")
	if p == "" { p = "8080" }
	r.Run(":" + p)
