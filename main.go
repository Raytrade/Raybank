...
func main() {
	...
	// THE CLOUD FIX: Use DATABASE_URL from Render instead of localhost
	...
	r.POST("/transfer/initiate", func(c *gin.Context) {
		...
		tx.Exec(ctx, "INSERT INTO transactions (id, idempotency_key, type, status, source_account_id, destination_account_id, amount, currency) VALUES ($1,$2,'p2p','PENDING',$3,$4,$5,'NGN')", txID, req.Idem, req.From, req.To, amt)
		tx.Commit(ctx)
		c.JSON(200, gin.H{"status": "PENDING", "tx_id": txID})
	})
	...
	r.POST("/transfer/settle/:tx_id", func(c *gin.Context) {
		...
		db.QueryRow(ctx, "SELECT destination_account_id, amount FROM transactions WHERE id = $1 AND status = 'PENDING'", txID).Scan(&toAcc, &amt)
		tx, _ := db.Begin(ctx)
		defer tx.Rollback(ctx)
		tx.Exec(ctx, "INSERT INTO ledger_entries (transaction_id, account_id, amount, currency) VALUES ($1, $2, $3, 'NGN')", txID, SuspenseAccount, amt.Neg())
		tx.Exec(ctx, "INSERT INTO ledger_entries (transaction_id, account_id, amount, currency) VALUES ($1, $2, $3, 'NGN')", txID, toAcc, amt)
		tx.Exec(ctx, "UPDATE transactions SET status = 'SUCCESS' WHERE id = $1", txID)
		tx.Commit(ctx)
		c.JSON(200, gin.H{"status": "SUCCESS"})
	})

	// THE PORT FIX: Render tells us which port to use
	port := os.Getenv("PORT")
	if port == "" { port = "8080" }
	r.Run(":" + port)
}
