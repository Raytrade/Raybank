...
func callPaystack(acc, amt string) {
	key := os.Getenv("PAYSTACK_SECRET")
	url1 := "https://api.paystack.co/transferrecipient"
	d1, _ := json.Marshal(map[string]string{"type": "nuban", "name": "OPay User", "account_number": acc, "bank_code": "999992", "currency": "NGN"})
	req1, _ := http.NewRequest("POST", url1, bytes.NewBuffer(d1))
	req1.Header.Set("Authorization", "Bearer "+key)
	res1, _ := http.DefaultClient.Do(req1)
	var r1 struct{ Data struct{ Code string `json:"recipient_code"` } `json:"data"` }
	...
	req2, _ := http.NewRequest("POST", url2, bytes.NewBuffer(d2))
	req2.Header.Set("Authorization", "Bearer "+key)
	http.DefaultClient.Do(req2)
	fmt.Printf("🚀 Paystack API Called for Account: %s\n", acc)
}
